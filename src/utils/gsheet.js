...
const getCSV = async (id) => {
  try {
    const res = await axios.get(
      `https://docs.google.com/spreadsheets/u/1/d/${id}/export?format=csv`
    );
    return res.data;
  } catch (error) {
...
const csvToJson = (csv = ``) => {
  ...
  const keys =
    rows?.[0]?.split(commaRegex).map((h) => h.replace(quotesRegex, "$1")) || [];
  const arr = [];
  for (const row of rows.slice(1)) {
    const obj = {};
    const cols =
      row?.split(commaRegex).map((h) => h.replace(quotesRegex, "$1")) || [];
